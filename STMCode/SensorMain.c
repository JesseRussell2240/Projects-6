...
uint32_t TxMailbox;
uint8_t TxData[8];
uint8_t msgBuffer[8]; // For writeOBD2()
...
int main(void)
{
  ...
  TxHeader.StdId = 0x6F0;  //CAN ID for sending temps
  TxHeader.RTR = CAN_RTR_DATA;
  TxHeader.DLC = 8;

  uint8_t rom1[8] = {0x28, 0xEA, 0xB2, 0x45, 0xD4, 0x21, 0x65, 0x8D};
  uint8_t rom2[8] = {0x28, 0x8C, 0xBB, 0x44, 0xD4, 0x65, 0x52, 0x1D};
  uint8_t rom3[8] = {0x28, 0x89, 0xA2, 0x45, 0xD4, 0xA6, 0x60, 0x15};

  DS18B20_SetROM(0, rom1);
  DS18B20_SetROM(1, rom2);
  DS18B20_SetROM(2, rom3);


//***ADD BACK TO READ ROM OF NEW SENSOR******//
    // uint8_t rom[8];
    // DS18B20_ReadROM(rom);
...
//*******************************************//
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	    ...
	    DS18B20_ReadTempIndexed(0, &temp1);
	    DS18B20_ReadTempIndexed(1, &temp2);
	    DS18B20_ReadTempIndexed(2, &temp3);

	    printf("Sensor 1: %.2fC\tSensor 2: %.2fC\tSensor 3: %.2fC\r\n", temp1, temp2, temp3);

	    writeOBD2Temp(temp1, 0x10); //sensor 1
	    writeOBD2Temp(temp2, 0x11); //sensor 2
	    writeOBD2Temp(temp3, 0x12); //sensor 3
